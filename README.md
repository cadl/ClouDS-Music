...
- 浏览公开新歌、每日推荐、自己创建或收藏的歌单以及音乐云盘
- 使用网易云音乐手机 App 扫码登录，并支持安全退出
- 搜索歌曲、歌手和专辑，内置本地拼音输入法
- 播放标准音质 MP3，支持云盘歌曲、播放队列、播放模式、自动续播和进度拖动
- 支持耳机合盖播放，并可选择启用合盖时 `L/R` 切换歌曲
- 从正在播放的歌曲查看专辑或艺人，并浏览艺人的专辑和热门歌曲
...
| `A` | 确认、打开或播放 |
| `B` | 返回或取消当前任务 |
| `L / R` | 切换 Tab；启用设置后可在插耳机合盖播放时切换歌曲 |
| `X / Y` | 执行当前页面显示的扩展操作 |
| `SELECT` | 切换上下屏焦点或播放模式 |
| `START` | 3 秒内再次按下以退出 |
| 触摸下屏 | 控制播放、进度、播放列表以及专辑/艺人入口 |

每个页面都会在下屏显示当前可用的按键提示，具体操作以屏幕提示为准。
正在播放时可以触摸下屏的“专辑”或“艺人”；合唱歌曲会先要求选择艺人。艺人页按
`Y` 切换专辑和热门歌曲；热门歌曲页按 `X` 可全部加入播放列表，`A` 打开专辑或
播放歌曲，`B` 按层返回。
“设置”中的“耳机合盖 L/R 切歌”默认关闭；开启后，仅在插着耳机、正在播放且合盖
时，`L` 播放上一首、`R` 播放下一首。未插耳机或暂停时仍按原有策略进入睡眠。
...
- 音乐云盘会显示上传文件的扩展名；云盘中常见的 MP3 和 FLAC 都可以列出，但本机
  解码器目前只支持 MP3。应用播放时会向网易云请求标准音质 MP3；只有服务端实际
  返回 MP3 时才能播放，服务端返回 FLAC、AAC 等其他格式时会在下载前明确报错。
